package com.xych.simple.spring.mvc.framework.v3.servlet;
...
import com.xych.simple.spring.mvc.framework.v3.Handler;
...
public class DispatcherServlet extends HttpServlet {
    ...
    private void doDispatch(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        paramMap.forEach((key, values) -> {
            if(handler.getParamIndexMapping().containsKey(key)) {
                Integer idx = handler.getParamIndexMapping().get(key);
                // 暂不考虑数组类型的参数
                params[idx] = values[0];
            }
        });
        if(handler.getParamIndexMapping().containsKey(HttpServletRequest.class.getName())) {
            params[handler.getParamIndexMapping().get(HttpServletRequest.class.getName())] = req;
        }
        if(handler.getParamIndexMapping().containsKey(HttpServletResponse.class.getName())) {
            params[handler.getParamIndexMapping().get(HttpServletResponse.class.getName())] = resp;
        }
    ...
    private void initHandlerMapping() {
        this.ioc.forEach((beanName, bean) -> {
            ...
            for(Method method : methods) {
                if(method.isAnnotationPresent(RequestMapping.class)) {
                    ...
                    this.handlerMapping.add(new Handler(pattern, method, bean));
